    ...
    return (
        <div>
            <StaticLatexSection>
                ...
                <p>
                    In a supervised learning task, the outputs of the final
                    layer <MathJax inline>{"\\(a^{[L]}\\)"}</MathJax> would then
                    represent the predicted value{" "}
                    <MathJax inline>{"\\(\\hat{y}\\)"}</MathJax> and carrying
                    out the learning process requires finding out the cost given
                    the current parameters on the training set.
                </p>

                <p>
                    Similar to before, the model learns the optimal parameters{" "}
                    <MathJax inline>{"\\(w^{[l]}\\)"}</MathJax> and{" "}
                    <MathJax inline>{"\\(b^{[l]}\\)"}</MathJax> so as to
                    minimize the cost function{" "}
                    <MathJax inline>
                        {"\\(J(w^{[0]},b^{[0]},w^{[1]},b^{[1]},\\ldots)\\)"}
                    </MathJax>
                    , where all parameters will be denoted simply as{" "}
                    <MathJax inline>{"\\(J(W)\\)"}</MathJax>. The optimal
                    parameters are denoted as{" "}
                    <MathJax inline>{"\\(W^*\\)"}</MathJax>. In a supervised
                    learning model, this can still be defined similar to before.
                </p>
                <div className="displayLatex">
                ...
                <p>
                    The algorithm for training remains similar to before. But
                    for neural networks, it is necessary to carry out{" "}
