...
const GradientDescentSection = (props) => {
    return (
        <div>
            <p>
                ...
                important to find the gradient of the Softmax output w.r.t to
                its inputs. This introduces more complexity given that the
                output is now a vector as opposed to a scalar.
            </p>

            <h3>Derivatives of the Softmax Function</h3>
            <p>
                To start, suppose that we want to derive{" "}
                <MathJax inline>{"\\(\\frac{S(z)_k}{z_j}\\)"}</MathJax> for any{" "}
                <MathJax inline>{"\\(j\\)"}</MathJax>,{" "}
                <MathJax inline>{"\\(k\\)"}</MathJax>. There are essentially two
                cases: either <MathJax inline>{"\\(j=k\\)"}</MathJax> or{" "}
                <MathJax inline>{"\\(j\\neq k\\)"}</MathJax>.
            </p>

            <p>
                In the scenario where <MathJax inline>{"\\(j=k\\)"}</MathJax>
            </p>
    ...
    \\end{align*}\\]`}</MathJax>
            ...
            <p>
                ...
                , these first-order partial derivatives can be represented in a
                Jacobian matrix{" "}
                <MathJax inline>
    ...
    \\end{bmatrix}\\]`}</MathJax>
            ...
            <p>
                ...
                each column{" "}
                <MathJax inline>{"\\(m\\in[1,\\ldots,n_c]\\)"}</MathJax> results
                in,
            </p>
            <div className="display-latex">
                <MathJax>{`\\[    \\begin{align*}
    ...
    \\end{align*}\\]`}</MathJax>
            ...
            <p>
                Suppose that we want to find the gradients of the *loss* w.r.t a
                particular input to the Softmax function{" "}
                <MathJax inline>{"\\(z^{[L]}_{k}\\)"}</MathJax>. Because{" "}
                <MathJax inline>{"\\(z^{[L]}_k\\)"}</MathJax> for all{" "}
                <MathJax inline>{"\\(k\\in[1,\\ldots,n_c]\\)"}</MathJax> is
                affects every component of the vector-valued function{" "}
                <MathJax inline>{"\\(S(z^{[L]})\\)"}</MathJax> then this partial
                derivative is essentially a sum across the partial derivatives
                of the loss w.r.t the value of all other neurons dependent on{" "}
                <MathJax inline>{"\\(z^{[L]}_j\\)"}</MathJax>. Similar to
                before, this sum can be decomposed to two cases where{" "}
                <MathJax inline>{"\\(t=k\\)"}</MathJax> and where{" "}
